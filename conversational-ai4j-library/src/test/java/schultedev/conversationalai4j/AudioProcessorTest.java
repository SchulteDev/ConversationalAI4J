...
class AudioProcessorTest {
  ...
  void testCombineAudioChunks() {
    ...
    var chunks = Arrays.asList(chunk1, chunk2, chunk3);
    var combined = AudioProcessor.combineAudioChunks(chunks);
  ...
  void testCombineEmptyChunks() {
    List<byte[]> emptyChunks = List.of();
    var combined = AudioProcessor.combineAudioChunks(emptyChunks);
  ...
  void testCombineWithNullChunks() {
    var chunksWithNull = Arrays.asList(new byte[] {1, 2}, null, new byte[] {3, 4});
    var combined = AudioProcessor.combineAudioChunks(chunksWithNull);
  ...
  void testConvertToFloatSamples() {
    ...
    var wavData = createSimpleWavData();

    var samples = AudioProcessor.convertToFloatSamples(wavData, AudioFormat.wav16kMono());
  ...
  void testConvertFromFloatSamples() {
    var samples = new float[] {0.5f, -0.5f, 0.0f, 1.0f, -1.0f};

    var audioData = AudioProcessor.convertFromFloatSamples(samples, AudioFormat.wav16kMono());
  ...
  void testPreprocessAudio() {
    ...
    var preprocessed = AudioProcessor.preprocess(audioData, format);
  ...
  void testConvertFormats() {
    ...
    var converted = AudioProcessor.convert(wavData, wavFormat, pcmFormat);
  ...
  void testConvertSameFormat() {
    ...
    var format = AudioFormat.rawPcm16kMono();

    var converted = AudioProcessor.convert(audioData, format, format);
  ...
  private byte[] createSimpleWavData() {
    ...
    var wavData = new byte[44 + 1000]; // Header + 500 samples
    ...
    for (var i = 44; i < wavData.length; i += 2) {
