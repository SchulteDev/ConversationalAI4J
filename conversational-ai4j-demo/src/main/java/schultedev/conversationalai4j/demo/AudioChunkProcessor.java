...
/**
 * Processes accumulated audio chunks through the complete AI pipeline:
 * Audio chunks → Combined audio → Speech-to-Text → LLM → Text-to-Speech → Response
 */
@Service
public class AudioChunkProcessor {
  ...
  public static class ProcessingResult {
    ...
    private ProcessingResult(String transcribedText, String aiResponse, byte[] responseAudio, 
    ...
    public static ProcessingResult success(String transcribedText, String aiResponse, byte[] responseAudio) {
    ...
    public byte[] getResponseAudio() { return responseAudio; }
  ...
  private ProcessingResult processAudioChunksSync(
    ...
    try {
      ...
      log.info(
          "Processing {} bytes of combined audio (format: {})",
      ...
      if (conversationalAI.isSpeechEnabled()) {
        try {
          ...
          if (responseAudio != null && responseAudio.length > 0) {
            log.info("TTS completed in {} ms, generated {} bytes", 
                ((t5 - t4) / 1_000_000), responseAudio.length);
          } else {
